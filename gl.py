...
class Render(object):
    ...
    def glLoadObj(self, filename, translate = [0, 0, 0], scale = [1, 1, 1]):
        ...
        # se define la luz en el medio del objeto
        light = [0, 0, 1]
        ...
        for face in model.faces:
            ...
            if intensity >= 0:
                self.point_color = color(intensity, intensity, intensity)
                self.glBaricentricTriangle(v0, v1, v2)
            ...
            if vertexes > 3:
                ...
                if intensity >= 0:
                    self.point_color = color(intensity, intensity, intensity)
                    self.glBaricentricTriangle(v0, v2, v3)

    # dibujado y rellenado de un triángulo usando coordenadas baricentricas
    def glBaricentricTriangle(self, vA, vB, vC):
            ...
            for y in range(minY, maxY + 1):
                ...
                if tripleCompare([u, v, w], [0, 0, 0]):
                    ...
                    if z > self.zbuffer[y][x]:
                        self.glVertexNDC(x, y, self.point_color)
                        self.zbuffer[y][x] = z
